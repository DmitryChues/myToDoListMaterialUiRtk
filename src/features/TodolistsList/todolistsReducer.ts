...
export const todolistReducer = (state = initialState, action: TodolistActionsType): TodolistDomainType[] => {
	...
	switch (type) {
		case 'SET-TODOLISTS':
			return payload.todolists.map(el => ({ ...el, filter: 'all', entityStatus: 'idle' }))
		case 'DELETE-TODOLIST':
			return state.filter(el => el.id !== payload.todolistId)
		case 'ADD-TODOLIST':
			return [{ ...payload.todolist, filter: 'all', entityStatus: 'idle' }, ...state]
		case 'CHANGE-TODOLIST-TITLE':
			return state.map(el => el.id === payload.todolistId ? { ...el, title: payload.title } : el)
		case 'CHANGE-TODOLIST-FILTER':
			return state.map(el => el.id === payload.todolistId ? { ...el, filter: payload.filter } : el)
		case 'CHANGE-ENTITY-STATUS':
			return state.map(el => el.id === payload.todolistId ? { ...el, entityStatus: payload.entityStatus } : el)
		default: return state
	}
}

export const deleteTodoListAC = (todolistId: string) => ({
	type: 'DELETE-TODOLIST' as const, payload: { todolistId, }
})
export const addTodoListAC = (todolist: TodolistType) => ({
	type: 'ADD-TODOLIST' as const, payload: { todolist }
})
export const changeTodoListTitleAC = (todolistId: string, title: string) => ({
	type: 'CHANGE-TODOLIST-TITLE' as const, payload: { todolistId, title, }
})
export const changeTodoListFilterAC = (todolistId: string, filter: FilterValuesType) => ({
	type: 'CHANGE-TODOLIST-FILTER' as const, payload: { todolistId, filter, }
})
export const changeEntityStatus = (todolistId: string, entityStatus: RequestStatus) => ({
	type: 'CHANGE-ENTITY-STATUS' as const, payload: { todolistId, entityStatus }
})
export const setTodoListAC = (todolists: TodolistType[]) => ({ type: 'SET-TODOLISTS' as const, payload: { todolists } })
...
export const changeTodoListTC = (title: string, todolistId: string): AppThunk => (dispatch) => {
	dispatch(setLoading('loading'))
...
export type TodolistActionsType = DeleteTodoListACType
	| AddTodoListACType
	| SetTodosACType
	| SetLoading
	| SetError
